...
const client = new Client({
  intents: [
    ...
    GatewayIntentBits.GuildMembers, // Added for username fetching
  ],
...
async function handleWalletCommand(interaction, walletAddress) {
  try {
    ...
    if (userError && userError.code !== 'PGRST116') {
      ...
      return interaction.reply('❌ Database error querying user data.', { ephemeral: true });
    }

    if (!walletAddress) {
      if (existingUser) {
        return interaction.reply(`Your current wallet is: \`${existingUser.wallet_address}\`\nTo update, use \`/mywallet <new_address>\`.`, { ephemeral: true });
      } else {
        return interaction.reply('❌ No wallet linked. Use `/mywallet <your_solana_address>` to link one.', { ephemeral: true });
      }
    }

    const walletRegex = /^[A-HJ-NP-Za-km-z1-9]{32,44}$/;
    if (!walletRegex.test(walletAddress)) {
      return interaction.reply('❌ Invalid Solana wallet address.', { ephemeral: true });
    }
    ...
    if (upsertError) {
      ...
      return interaction.reply('❌ Failed to save wallet due to database error.', { ephemeral: true });
    }

    const action = existingUser ? 'updated' : 'linked';
    await interaction.reply(`✅ Wallet ${action}: \`${walletAddress}\``, { ephemeral: true });
  } catch (error) {
    console.error('handleWalletCommand error:', error);
    if (!interaction.replied && !interaction.deferred) {
      await interaction.reply('❌ An unexpected error occurred.', { ephemeral: true });
    }
  }
}
...
async function handleVerifyCommand(interaction) {
  ...
  try {
    ...
    if (userError || !userData?.wallet_address) {
      return interaction.editReply(`❌ Please link your wallet first with \`/mywallet <your_solana_address>\`!`);
    }
    ...
    if (tokenError || !tokens || tokens.length === 0) {
      return interaction.editReply('❌ No active prize tokens available. Try again later.');
    }
    ...
    if (recentSpins.length >= dailySpinLimit) {
      ...
      return interaction.editReply(`❌ You've reached your daily spin limit for this token. Try again tomorrow!`);
    }
    ...
    await interaction.editReply('Your spin link is ready!');
    ...
    if (!interaction.replied && !interaction.deferred) {
        await interaction.reply(`❌ Failed to generate spin link.`, { ephemeral: true });
    } else {
        await interaction.editReply(`❌ Failed to generate spin link.`);
    }
  }
}
...
async function fetchLeaderboardText() {
  try {
    ...
    // Parse leaderboard text and replace wallet addresses with usernames
    ...
    const updatedLines = await Promise.all(lines.map(async (line) => {
      const match = line.match(/: ([A-HJ-NP-Za-km-z1-9]{32,44}) —/);
      if (!match) return line;
      const wallet = match[1];
      
      // Get discord_id from users table
      const { data: userData, error: userError } = await supabase
        .from('users')
        .select('discord_id')
      ...
      if (userError || !userData) return line; // Fallback to wallet if no user found
      ...
      try {
        ...
        return line; // Fallback to wallet if fetch fails
      }
    }));
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (["spin", "freespin", "dailyspin"].includes(interaction.commandName)) {
    if (interaction.channel.name !== SPIN_CHANNEL_NAME) {
      return interaction.reply(`Please use this command in the #${SPIN_CHANNEL_NAME} channel.`, { ephemeral: true });
    }
  ...
  if (["leaders", "leaderboard", "spinleaders"].includes(interaction.commandName)) {
    ...
    await interaction.editReply(`🏆 **Current Top 10 Winners:**\n\n${leaderboard}`);
  }
