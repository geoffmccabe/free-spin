...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ⬇️ IMPORTANT: only block "spin" with used tokens; allow admin/config reads.
    if (spin && tokenData.used) {
      console.error(`Token already used for spin: ${signedToken}`);
      ...
      if (role === 'admin' || role === 'superadmin') {
        try {
          ...
          if (COINMARKETCAP_API_KEY) {
            ...
            try {
              // Spin token price (symbol from config.token_name, if listed)
