     ...
     export default async function handler(req, res) {
       ...
       try {
         ...
         const { data: limitRow, error: limitError } = await supabase
           ...
           .eq('wallet_address', tokenRow.discord_id)
           .single();

         const dailySpinLimit = limitRow?.daily_spin_limit || 1; // Default to 1 if no limit set
         ...
         let userWallet;
         try {
           userWallet = new PublicKey(tokenRow.discord_id);
         } catch (err) {
           console.error("Invalid user wallet address:", tokenRow.discord_id, err.message);
